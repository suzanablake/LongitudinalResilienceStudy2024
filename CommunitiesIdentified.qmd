...
new_folder_path <- "~/Resilience/UpdatedData"
...
com_names[str_detect(com_names, regex("grouper", ignore_case = T))]# pulls out everything that contains shrimp in it regardless of capitalization
...
com_names[str_detect(com_names, regex("tuna", ignore_case = T))]
com_names[str_detect(com_names, regex("snapper", ignore_case = T))]
com_names[str_detect(com_names, regex("drum", ignore_case = T))]

BWP_shrimp <- c("Brown Shrimp",  "White Shrimp","Pink Shrimp")
Other_shrimp <- c("SHRIMP, ATLANIC SEABOB", "SHRIMP, ROYAL RED", "SHRIMP, ROCK", "SHRIMPS, PENAEOID", "SHRIMP, ROUGHNECK", "SHRIMP, BAIT", "SHRIMP, PISTOL OR SNAPPING", "SHRIMP, BANDED CORAL", "SHRIMP, PEPPERMINT", "SHRIMP, CLEANER", "SHRIMP, SPOTTED CLEANER", "SHRIMP, MARINE", "SHRIMP, MANTIS","SHRIMP,ROCK", "SHRIMP,FW", "SHRIMP, FARFANTEPENAEUS SPP","SHRIMP, PEDERSON CLEANER", "SHRIMP, GOLDEN CORAL", "SHRIMPS, BAIT", "SHRIMPS, PISTOL OR SNAPPING", "SHRIMPS, MANTIS")
Grouper <- c("GROUPERS", "GOLIATH GROUPER", "GROUPER, WARSAW","GROUPER, GAG", "GROUPER, YELLOWEDGE","GROUPER, RED","GROUPER, BLACK", "GROUPER, YELLOWFIN", "GROUPER, SNOWY","GROUPER, NASSAU","GROUPER, YELLOWMOUTH","GROUPER, MISTY", "GROUPER, TIGER","GROUPER, MARBLED", "GROUPER, HIND RED","GROUPER, SCAMP", "GROUPER, SPECKLED HIND", "GROUPER, CONEY","GROUPERS, SERRANIDAE (FAMILY)")
Tuna <- c("TUNA", "TUNA, BLACKFIN", "TUNA, BLUEFIN","TUNA, YELLOWFIN", "TUNA, BIGEYE", "TUNA, SKIPJACK","TUNA, LITTLE TUNNY", "TUNA, ALBACORE")
Snapper <- c("SNAPPER, RED","SNAPPER, GRAY","SNAPPER, VERMILION","SNAPPER, MUTTON","SNAPPER, LANE","SNAPPER, YELLOWTAIL", "SNAPPERS","SNAPPER, SILK", "SNAPPER, CUBERA", "SNAPPER, BLACKFIN", "SNAPPER, QUEEN","SNAPPER, GLASSEYE", "SNAPPER, DOG", "SNAPPER, BLACK", "SNAPPER, MAHOGANY","SNAPPER, CARIBBEAN RED","SNAPPERS, LUTJANIDAE (FAMILY)")

GI$group[GI$common_name %in% BWP_shrimp]<-"BWP Shrimp"
GI$group[GI$common_name %in% Other_shrimp]<-"Other Shrimp"
GI$group[GI$common_name %in% Grouper]<-"Grouper"
GI$group[GI$common_name %in% Tuna]<-"Tuna"
GI$group[GI$common_name %in% Snapper]<-"Snapper"
#GOM$group[GOM$common_name %in% Snapper]<-"Snapper"

# check this page for more tips on strings and piping https://stringr.tidyverse.org/

#Shrimp <- com_names[str_detect(com_nms, regex("shrimp", ignore_case = T))] 
#Grouper <- com_names[str_detect(com_names, regex("grouper", ignore_case = T))]

#GOM$group[test$common_name %in% Shrimp2]<-"shrimp" # we create a column named "group" and placed in it everything that contains the word shrimp
#GOM$group[test$common_name %in% Grouper]<-"grouper"
